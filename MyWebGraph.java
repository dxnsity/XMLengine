...
public class MyWebGraph {
    ...
    if (vertexList.containsKey(s) && vertexList.containsKey(t)) {
        ...
        WebVertex source = vertexList.get(s);
        WebVertex target = vertexList.get(t);
	...
	public ArrayList<String> getEdgesInto(String v) {
    	ArrayList<String> edgesIntoList = new ArrayList<>();
    for (String url : vertexList.keySet()) {
        if (vertexList.get(url).containsEdge(v)) {
            edgesIntoList.add(url);
        }
    }
    return edgesIntoList;
}
